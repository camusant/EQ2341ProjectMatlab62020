...
q=mc.InitialProb; 
A=mc.TransitionProb;
infin=~mc.finiteDuration; %HMM with infinite duration ?
betaHat=zeros(nS,T); 

%Initialization
if infin    %Infinite duration case
    betaHat(:,T)=1/c(T); %eq(5.64)
else        %Finite duration case
    for i=1:nS
        betaHat(i,T)=A(i,nS+1)/(c(T)*c(T+1)); %eq(5.65)
    end
end
%Backward step
for t=T-1:-1:1
    for i=1:nS
        summ=0;
        for j=1:nS
            summ=A(i,j)*pX(j,t+1)*betaHat(j,t+1)+summ; 
        end
        betaHat(i,t)=(1/c(t))*summ; %eq(5.66)
    end
end
